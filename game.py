...
class Game:
   ...
   def __check_hallway(self,token,dice):
      ...
      for i,j in self.__table.get_exit_halls():
         ...
         elif token.get_colour()=='B' and index>=j-5 and index<=j+1:
            index=token.get_position()+dice-j
            return True,index    
         elif token.get_colour()=='R' and index>=j-5 and index<=j+1:
            index=token.get_position()+dice-j
            return True,index
         elif token.get_colour()=='G' and index>=j-5 and index<=j+1:
            index=token.get_position()+dice-j
            return True,index      
   ...
   def __move(self,id:int,dice):
     token=self.__token[self.__get_token(id)]
     if token.get_state()=='InGame' or token.get_state()=='Safe':
         if token.get_position()+ dice>=68:
